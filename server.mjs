...
app.get('/',(_,res)=>res.json({ok:true,service:'land-registry-parser',version:'1.2.1-elite-parity'}));
...
async function extractPdfText(file){
  ...
  if(!name.toLowerCase().endsWith('.pdf')) throw Object.assign(new Error('僅接受 PDF 檔案。'),{status:415});
  ...
  try{
    ...
    return {text:normalizeTranscriptLabels(composePages(pages)),pageCount:pdf.numPages,name};
  } finally { await pdf.destroy(); }
}
function normalizePropertyNo(value){ return String(value||'').replace(/\D/g,''); }
function looksLikeIndexOnly(item){
  if(/IDX|異動/i.test(item.name)) return true;
...
async function parseBatch(files){
  ...
  for(const file of files){
    const {text,pageCount,name}=await extractPdfText(file);
    extracted.push({file,text,pageCount,name});
  }
  const indexMap=new Map();
  const indexFiles=[];
  for(const item of extracted){
    if(!isChangeIndexText(item.text,item.name)) continue;
    const propertyNo=extractNoFromIndex(item.text);
    const records=parseChangeIndexData(item.text,{name:item.name});
    ...
    indexFiles.push({filename:item.name,property_no:propertyNo,record_count:records.length,needs_ocr:records.some(r=>!r.rawHolder||r.rawHolder==='(空白)')});
  }
  const rows=[];
  const documents=[];
  const warnings=[];
  for(const item of extracted){
    ...
    const parsedList=parseRealEstateDataList(item.text,item.name);
...
app.post('/api/parse',upload.single('file'),async(req,res)=>{
  try{
    ...
    return res.json({ok:true,parser_version:'1.2.1-elite-parity',filename:uploadName(req.file),file_size:req.file.size,...result,privacy:'PDF 僅在本次請求記憶體中處理，API 不主動保存原始檔。'});
  } catch(error){
    console.error(error);
...
app.post('/api/parse-batch',upload.array('files',maxFiles),async(req,res)=>{
  try{
    ...
    return res.json({ok:true,parser_version:'1.2.1-elite-parity',file_count:files.length,total_size:files.reduce((s,f)=>s+f.size,0),...result,privacy:'PDF 僅在本次請求記憶體中處理，API 不主動保存原始檔。'});
  } catch(error){
    console.error(error);
    const status=error?.status || (error?.code==='LIMIT_FILE_SIZE'?413:422);
