...
function extractNoFromTranscriptLines(lines,isLand){
  const label=isLand?"地號":"建號"; for(const line of lines){ if(!line.includes(label)) continue; const m=line.match(isLand?/(\d{4})\s*-\s*(\d{4})/:/(\d{5})\s*-\s*(\d{3})/); if(m) return `${m[1]}-${m[2]}`; const d=line.match(/(\d{8})/); if(d) return formatPropertyNo(d[1],isLand); } return "";
}
...
function extractAndGroupCollateral(text){
  const t=normalizeTranscriptLabels(text); if(!/共同擔保(?:地號|建號)\s*:/.test(t)) return [];
  const rights=sliceSection(t,["土地他項權利部","建物他項權利部","他項權利部"],["異動索引"]);
  return rights.split(/(?<![\u4e00-\u9fa5])登記次序\s*[:：]?/).slice(1).map((entry,index)=>{
    const found=[]; for(const m of entry.matchAll(/共同擔保(地號|建號)\s*:\s*([\s\S]*?)(?=共同擔保(?:地號|建號)\s*:|其他登記事項\s*:|本謄本|$)/g)){
      let section=""; for(const token of m[2].matchAll(/([\u4e00-\u9fa5A-Za-z]+段(?:[\u4e00-\u9fa5A-Za-z]+小段)?)|(\d{4,5}-\d{3,4}|\d{8})/g)){ if(token[1]) section=token[1]; else if(section) found.push(`${section}${m[1]}:${formatPropertyNo(token[2],m[1]==="地號")}`); }
    }
    return {label:`H${index+1}`,content:[...new Set(found)].join("、")};
  }).filter(g=>g.content);
}
...
export function parseRealEstateData(text, filename="") {
  ...
  const type=isLand?"土地":"建物"; const streamText=normalizedText.replace(/\s+/g," "); const ownershipText=sliceSection(streamText,[isLand?"土地所有權部":"建物所有權部","所有權部"],[isLand?"土地他項權利部":"建物他項權利部","他項權利部","異動索引"]); const rightsText=sliceSection(streamText,[isLand?"土地他項權利部":"建物他項權利部","他項權利部"],["異動索引"]);
  const result={owner:"",ownerId:"",city:"",dist:"",location:"",mainNo:"",areaA:"0",mainBuildingParts:[],publicParts:[],parkingCount:"",parkingRatio:"",settings:[],notes:{type,dateBuy:"",dateBuild:"",address:"",usage:"",floorInfo:"",parking:"無",material:"",announcedValue:"",announcedPrice:"",regReason:"",unknownRights:"",riskAlerts:""},history:[],pureAmounts:[],collateralGroups:extractAndGroupCollateral(text),rawBuildNoForLink:"",queryTime:"-",sourceFile:filename};
  const tm=streamText.match(/(?:列印|核發|謄本列印|資料日期|發給日期)\s*(?:時間|日期)?\s*[:：]?\s*(?:民國)?(\d{2,3})年(\d{1,2})月(\d{1,2})日/); if(tm) result.queryTime=`${String(tm[1]).padStart(3,"0")}/${String(tm[2]).padStart(2,"0")}/${String(tm[3]).padStart(2,"0")}`;
  const risks=getMatchedRiskKeywords(streamText); result.notes.riskAlerts=[...new Set(risks.map(r=>r==="流抵"?"出現流抵文字請確認":r))].join("、");
  if(ownershipText){ let area=ownershipText; const p=ownershipText.indexOf("登記次序"); if(p!==-1) area=ownershipText.substring(p); const om=area.match(/所有權人\s*[:]?\s*([^\s:管理者統編]+)/); if(om){ const rm=area.match(/權利範圍\s*[:]?\s*([^\s]+)/); result.owner=`${om[1]} (${normalizeRightsRatio(rm?rm[1]:"全部")})`; } const id=area.match(/(?:所有權人證號|身分證統一編號|統一編號|所有權人統一編號)\s*[:]?\s*([A-Z0-9*＊Xx]+|\S+)/); if(id) result.ownerId=id[1].replace(/[：:]/g,"").trim(); const reason=ownershipText.match(/登記原因\s*[:]?\s*([^\s]+)/); if(reason) result.notes.regReason=reason[1]; const rd=ownershipText.match(/登記日期\s*[:]?\s*(?:民國)?\s*(\d+年\d+月\d+日)/); if(rd) result.notes.dateBuy=formatDate(rd[1]); }
  ...
  if(isLand){ const mark=sliceSection(streamText,["土地標示部","標示部"],["所有權部"]); const a=extractAreaNumber(mark); if(a){result.areaA=String(Number(a)); result.mainBuildingParts.push({type:"土地",area:result.areaA});} const zone=mark.match(/使用分區\s*[:]?\s*(\S+)/); const cls=mark.match(/使用地類別\s*[:]?\s*(\S+)/); result.notes.usage=[zone?`分區:${zone[1].replace(/（空白）/g,"(空白)")}`:"",cls?`類別:${cls[1].replace(/（空白）/g,"(空白)")}`:""].filter(Boolean).join(" "); const av=mark.match(/公告(?:土地)?現值\s*[:]?\s*.*?([\d,]+)\s*元/); const ap=mark.match(/公告地價\s*[:]?\s*.*?([\d,]+)\s*元/); if(av)result.notes.announcedValue=av[1]; if(ap)result.notes.announcedPrice=ap[1]; const gb=streamText.match(/地上建物(?:\(建號\)|建號)?[:：]?\s*共?\s*\d+[筆棟]?\s*([\d\s]+)/); if(gb){const nums=gb[1].match(/\d{8,9}/g)||[]; result.notes.address=nums.map(formatBuildNo).join("、");}
  } else { const addr=streamText.match(/建物門牌\s*[:]?\s*([^\s]+)/); if(addr) result.notes.address=addr[1]; const mark=sliceSection(streamText,["建物標示部","標示部"],["所有權部"]); const a=extractAreaNumber(mark); if(a){result.areaA=String(Number(a)); result.mainBuildingParts.push({type:"主建物",area:Number(a).toFixed(2)});} const parking=extractParkingDetails(streamText); result.parkingCount=parking.count; result.parkingRatio=parking.ratio; }
  if(rightsText){ const entries=rightsText.split(/(?<![\u4e00-\u9fa5])登記次序\s*[:：]?/).slice(1); let h=1; for(const entry of entries){ if(!(entry.includes("抵押權")||entry.includes("最高限額"))) continue; const d=entry.match(/登記日期\s*[:]?\s*(?:民國)?\s*(\d+年\d+月\d+日)/); const holder=entry.match(/權利人\s*[:]?\s*([^\s:0-9]+?)(?=\s*(?:權利人統一編號|統一編號|住址|債權額|擔保債權|權利範圍|設定|$))/); const amt=entry.match(/(?:擔保債權總金額|債權額比例|最高限額|債權額).*?新[台臺]幣[\s*＊]*([0-9,]+)/); if(d) result.settings.push(`H${h++}-${formatDate(d[1])} ${holder?holder[1]:"未知"} ${amt?amt[1]:""}`.trim()); } }
  return result;
}

function stripHtml(value){ return String(value||"").replace(/<[^>]+>/g,"").replace(/&nbsp;/g," ").trim(); }
export function toOfflineRows(data,indexData={}){
  ...
  const shareDisplay=(share*100).toFixed(2)+"%"; const a=Number(data.areaA)||0; const aPing=(a*0.3025); let pub=0; for(const p of data.publicParts||[]){const[n,d]=String(p.ratio||"").split("/").map(Number); if(d)pub+=Number(p.area||0)*(n/d);} let parking=0; if(data.parkingRatio&&data.parkingRatio.includes("/")){const[n,d]=data.parkingRatio.split("/").map(Number); const target=(data.publicParts||[]).find(p=>Number(String(p.ratio||"").split("/")[1])===d); if(target&&d)parking=Number(target.area||0)*(n/d);} const total=a+pub, noCar=total-parking;
  const mainParts=(data.mainBuildingParts||[]).map((p,i)=>`${i+1}. ${p.type}: ${p.area}m²`).join("\n")||"--"; const publicParts=(data.publicParts||[]).map((p,i)=>`共有${i+1}: ${Number(p.area).toFixed(2)} m², 持分 ${p.ratio} (建號 ${formatBuildNo(p.no)})`).join("\n")||"--";
  const settings=(data.settings||[]).map(stripHtml).join("\n")||"無"; const pure=(data.pureAmounts||[]).join("\n")||"無"; const collateral=(data.collateralGroups||[]).map(x=>`${x.label} ${x.content}`).join("\n")||"無";
