...
const LEASING_KEYS = ["中租","和潤","裕融","裕富","新鑫","萬泰租賃","歐力士","遠信","一銀租賃","華南國際租賃","國票金租賃","永豐金租賃","遠銀國際租賃","賓士資融","中華資融","凱基租賃","陽信國際租賃","台中銀租賃","臺灣企銀國際租賃","中泰租賃","匯豐協新租賃","板信國際租賃","合作金庫資產管理","日盛台駿"];
...
function findFirstIndex(text, keywords, startAt=0) {
  let best=-1; for (const k of keywords) { const i=text.indexOf(k,startAt); if(i!==-1&&(best===-1||i<best)) best=i; } return best;
}
function sliceSection(text, startKeywords, endKeywords) {
  const start=findFirstIndex(text,startKeywords); if(start===-1) return ""; const end=findFirstIndex(text,endKeywords,start+1); return text.substring(start,end!==-1?end:text.length);
}
function normalizeRightsRatio(rawRatio) {
  let ratio=String(rawRatio||"").replace(/[*＊]/g,"").trim();
...
function extractLocationFromTranscript(streamText){
  ...
  const cityDist=source.match(/([\u4e00-\u9fa5]{2,3}[縣市])\s*([\u4e00-\u9fa5]{1,4}[鄉鎮市區])/);
  let sections=[...source.matchAll(/([\u4e00-\u9fa5A-Za-z0-9]{1,12}段(?:[\u4e00-\u9fa5A-Za-z0-9]{1,12}小段)?)/g)].map(m=>m[1]).filter(v=>v!=="地段"&&v!=="路段");
  if(!sections.length) sections=[...streamText.matchAll(/([\u4e00-\u9fa5A-Za-z0-9]{1,12}段(?:[\u4e00-\u9fa5A-Za-z0-9]{1,12}小段)?)/g)].map(m=>m[1]).filter(v=>v!=="地段"&&v!=="路段");
  const authority=streamText.match(/資料管轄機關\s*[:：]\s*([\u4e00-\u9fa5]{2,3}[縣市])/);
  return {city:cityDist?cityDist[1]:(authority?authority[1]:""),dist:cityDist?cityDist[2]:"",section:sections[0]||""};
}
function extractOfficialOwner(ownershipText){
  if(!ownershipText) return {owner:"",ownerId:"",ratio:"1/1"};
  let searchArea=ownershipText; const idx=ownershipText.indexOf("登記次序"); if(idx!==-1) searchArea=ownershipText.substring(idx);
  ...
  return {owner:ownerMatch?ownerMatch[1].trim():"",ownerId:idMatch?idMatch[1].trim():"",ratio:normalizeRightsRatio(ratioMatch?ratioMatch[1]:"全部")};
}
function extractParkingDetails(streamText){
  ...
  const remaining=streamText.substring(h.index+h[0].length);
  const boundaryCandidates=[];
  const sectionEnd=remaining.match(/(其他登記事項|建物所有權部|權利種類|建築基地權利)/); if(sectionEnd) boundaryCandidates.push(sectionEnd.index);
  const commonStart=remaining.match(/(?:\S+段)\s+\d{8}\s+\d+\s*分之\s*\d+\s+[\d,.]+\s*平方公尺/); if(commonStart) boundaryCandidates.push(commonStart.index);
  const boundary=boundaryCandidates.length?Math.min(...boundaryCandidates):Math.min(remaining.length,2000);
  ...
  if(!matches.length){
    matches=[...context.matchAll(/(\S+)\s+(\d+)\s*分之\s*(\d+)/g)].filter(m=>{const token=(m[1]||"").trim(); return token&&!/^\d{8}$/.test(token)&&!token.includes("段");});
  }
  matches.forEach(m=>{count++; totalNum+=Number(m[3]); den=Number(m[2]);});
  return {count:count?String(count):"",ratio:count?`${totalNum}/${den}`:""};
}
function extractAndGroupCollateral(text){
  ...
  const rawGroups=rights.split(/(?<![\u4e00-\u9fa5])登記次序\s*[:：]?/).slice(1).map(entry=>{
    const segments={};
  ...
  const unique=[...new Set(rawGroups)];
  return unique.map((content,index)=>({label:`H${index+1}`,content}));
}
...
function classifySettingSource(holder){
  const text=String(holder||"");
  if(BANK_SETTING_KEYWORDS.some(k=>text.includes(k))) return "銀行設定";
  if(LEASING_KEYS.some(k=>text.includes(k))) return "租賃公會設定";
  if(/股份有限公司|有限公司|公司|法人/.test(text)) return "民間法人設定";
  if(text && text!=="未知") return "民間個人設定";
  return "";
}
...
export function splitOfficialTranscriptText(text){
  ...
  while((m=re.exec(source))!==null){
    const pb=source.lastIndexOf("[PAGE_BREAK]",m.index);
    const lineStart=source.lastIndexOf("\n",m.index);
    const s=pb!==-1?pb+"[PAGE_BREAK]".length:(lineStart!==-1?Math.max(0,source.lastIndexOf("\n",lineStart-1)):0);
...
export function parseRealEstateData(text, filename="") {
  ...
  const isLand=compact.includes("地號查詢土地資料")||compact.includes("土地標示部")||compact.includes("土地登記謄本")||/土地.{0,30}登記謄本/.test(compact)||/登記謄本.{0,30}土地/.test(compact);
  const type=isLand?"土地":"建物";
  const streamText=normalizedText.replace(/\s+/g," ");
  ...
  const risks=getMatchedRiskKeywords(streamText);
  result.notes.riskAlerts=[...new Set(risks.map(r=>r==="流抵"?"出現流抵文字請確認":r))].join("、");

  if(ownershipText){
    const officialOwner=extractOfficialOwner(ownershipText);
    if(officialOwner.owner) result.owner=`${officialOwner.owner} (${officialOwner.ratio})`;
    if(officialOwner.ownerId) result.ownerId=officialOwner.ownerId;
    ...
    else { const occur=ownershipText.match(/發生日期\s*[:]?\s*(?:民國)?\s*(\d+年\d+月\d+日)/); if(occur) result.notes.dateBuy=formatDate(occur[1]); }
  }
  ...
  if(isLand){
    const mark=sliceSection(streamText,["土地標示部","標示部"],["所有權部"]);
    const a=extractAreaNumber(mark); if(a){result.areaA=String(Number(a)); result.mainBuildingParts.push({type:"土地",area:result.areaA});}
    const zone=mark.match(/使用分區\s*[:]?\s*(\S+)/); const cls=mark.match(/使用地類別\s*[:]?\s*(\S+)/);
    result.notes.usage=[zone?`分區:${zone[1].replace(/（空白）/g,"(空白)")}`:"分區:(空白)",cls?`類別:${cls[1].replace(/（空白）/g,"(空白)")}`:"類別:(空白)"].join(" ");
    ...
    const usageScope=(streamText.match(/建物門牌[\s\S]*?主要(?:建築材料|建材)/)||[])[0]||streamText;
    const usage=usageScope.match(/主要用途\s*[:：]?\s*([^\s]+)/)||streamText.match(/主要用途\s*[:：]?\s*([^\s]+)/); if(usage) result.notes.usage=usage[1];
  ...
  if(rightsText){
    const entries=rightsText.split(/(?<![\u4e00-\u9fa5])登記次序\s*[:：]?/).slice(1);
    const seenSettings=new Set(); let h=1;
    for(const entry of entries){
      const hasMortgage=entry.includes("抵押權")||entry.includes("最高限額")||/權利種類\s*[:：]\s*\S*抵押權/.test(entry);
      const hasSetting=entry.includes("設定")||hasMortgage;
      if(!hasMortgage||!hasSetting) continue;
      const d=entry.match(/登記日期\s*[:]?\s*(?:民國)?\s*(\d+年\d+月\d+日)/);
      if(!d) continue;
      const holder=entry.match(/權利人\s*[:]?\s*([^\s:0-9]+?)(?=\s*(?:權利人統一編號|統一編號|住址|債權額|擔保債權|權利範圍|設定|$))/);
      const holderName=holder?holder[1]:"未知";
      const amt=entry.match(/(?:擔保債權總金額|債權額比例|最高限額|債權額).*?新[台臺]幣[\s*＊]*([0-9,]+)/);
      const raw=amt?parseInt(amt[1].replace(/,/g,""),10):0;
      const display=raw?`${Math.round(raw/1000).toLocaleString()}仟元`:"";
      const cg=result.collateralGroups[0]||result.collateralGroups.find(g=>g.label===`H${h}`);
      const sourceLabel=classifySettingSource(holderName);
      const dedupe=[formatDate(d[1]),holderName,display,cg?.content||""].join("|");
      if(seenSettings.has(dedupe)) continue;
      seenSettings.add(dedupe);
      result.settings.push(`${sourceLabel}H${h}-${formatDate(d[1])} ${holderName}${display?` ${display}`:""}${cg?` (共擔: ${cg.content})`:""}`.trim());
      h++;
    }
  }
  return result;
...
export function toOfflineRows(data){
  const om=(data.owner||"").match(/([^(]+)\s*\(([^)]+)\)/);
  const ownerName=om?om[1].trim():data.owner;
  const ratio=om?om[2]:"1/1";
  let share=1; if(ratio.includes("/")){const[n,d]=ratio.split("/").map(Number);if(d)share=n/d;}
  const shareDisplay=(share*100).toFixed(2)+"%";
  const a=Number(data.areaA)||0; const aPing=a*0.3025;
  let pub=0;
  for(const p of data.publicParts||[]){const[n,d]=String(p.ratio||"").split("/").map(Number);if(d)pub+=Number(p.area||0)*(n/d);}
  let parking=0;
  if(data.parkingRatio&&data.parkingRatio.includes("/")){
    const[n,d]=data.parkingRatio.split("/").map(Number);
    const target=(data.publicParts||[]).find(p=>Number(String(p.ratio||"").split("/")[1])===d);
    if(target&&d)parking=Number(target.area||0)*(n/d);
  }
  ...
  const pure=(data.pureAmounts||[])[0]||"無";
  const collateral=(data.collateralGroups||[]).slice(0,1).map(x=>`${x.label} ${x.content}`).join("\n")||"無";
  const notes=[
    `1.所有權登記日: ${data.notes.dateBuy||"-"}`,
    `2.完工登記日: ${data.notes.dateBuild||"-"}`,
    `3.建築類型: ${data.notes.usage||"-"}`,
    `4.有無車位: ${Number(data.parkingCount||0)>0?"含車位":"無"}`,
    `5.主要建材: ${data.notes.material||"-"}`,
    `6.登記原因: ${data.notes.regReason||"-"}`,
    `7.風險提醒: ${data.notes.riskAlerts||"-"}`
  ].join(" ");
  return [{
    pre_review_case_no:"-",type:data.notes.type,owner_name:ownerName,owner_id:data.ownerId||"請重新確認",owner_share:shareDisplay,
    location:data.location,address:data.notes.address||"",main_no:data.mainNo,main_building_parts:mainParts,main_building_m2:a.toFixed(2),main_building_ping:aPing.toFixed(2),
    public_parts:publicParts,public_m2:pub.toFixed(2),public_ping:(pub*0.3025).toFixed(2),parking_count:data.parkingCount||"-",parking_ratio:data.parkingRatio||"-",
    parking_m2:parking>0?parking.toFixed(2):"-",parking_ping:parking>0?(parking*0.3025).toFixed(2):"-",total_m2_w_car:total.toFixed(2),total_ping_w_car:(total*0.3025).toFixed(2),
    total_m2_no_car:noCar.toFixed(2),total_ping_no_car:(noCar*0.3025).toFixed(2),announced_value:data.notes.announcedValue||"-",announced_price:data.notes.announcedPrice||"-",
    post_share_m2_w_car:(total*share).toFixed(2),post_share_ping_w_car:(total*0.3025*share).toFixed(2),post_share_m2_no_car:(noCar*share).toFixed(2),post_share_ping_no_car:(noCar*0.3025*share).toFixed(2),
    pure_amount:pure,collateral,collateral_groups:data.collateralGroups||[],settings,notes,change_index:`無關聯索引 (${data.rawBuildNoForLink||data.mainNo})`,query_time:data.queryTime,
    source_file:`${data.notes.type}: ${data.sourceFile}`,record_time:"-",risk_alerts:data.notes.riskAlerts?data.notes.riskAlerts.split("、"):[]
  }];
}
